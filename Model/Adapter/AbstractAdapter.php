                ...
                if ($resourceItemTypeId === self::PRODUCT_TYPE_CONFIGURABLE) {
                    ...
                    if (!empty($usedProducts)) {
                        foreach ($usedProducts as $usedProduct) {
                            ...
                            $cid = $usedProduct->getID();
                            $tierPrices = $usedProduct->getTierPrice();
                            if (!empty($tierPrices)) {
                                foreach ($tierPrices as $tierPrice) {
                                    if (!in_array($cid, $tierPriceValues)) {
                                        $tierPriceValues[$cid] = [];
                                    }
                                    if (!in_array($cid, $tierPriceQuantities)) {
                                        $tierPriceQuantities[$cid] = [];
                                    }
                                    if (isset($tierPrice['price'])) {
                                        $tierPriceValues[$cid][] = (float)number_format((float)$tierPrice['price'], 2, ".", "");
                                    }
                                    if (isset($tierPrice['price_qty'])) {
                                        $tierPriceQuantities[$cid][] = (int)$tierPrice['price_qty'];
                                    }
                                }
                            }
                            if (!empty($tierPriceValues[$cid]) && !empty($tierPriceQuantities[$cid])) {
                                $info['tpv_' . (string)$cid] = $tierPriceValues[$cid];
                                $info['tpq_' . (string)$cid] = $tierPriceQuantities[$cid];
                            }
                    ...
                    if ($resourceItemTypeId == self::PRODUCT_TYPE_CONFIGURABLE) {
                        $childProducts = $resourceItemTypeInstance->getUsedProducts($item);
                        if (!empty($childProducts)) {
                            foreach ($childProducts as $childProduct) {
                                $cid = $childProduct->getID();
                                $tierPrices = $childProduct->getTierPrice();
                                if (!empty($tierPrices)) {
                                    foreach ($tierPrices as $tierPrice) {
                                        if (!in_array($cid, $tierPriceValues)) {
                                            $tierPriceValues[$cid] = [];
                                        ...
                                        if (isset($tierPrice['price'])) {
                                            $tierPriceValues[$cid][] = $this->formatPrice($tierPrice['price']);
                                        }
                                        if (isset($tierPrice['price_qty'])) {
                                            $tierPriceQuantities[$cid][] = (int)$tierPrice['price_qty'];
                                        }
                    ...
                    // TODO: Move outside field handler to suffix attribute with variant id.
